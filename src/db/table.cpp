...
void TaskWorker::addTask(TableTask hand) {
  task_vec_mtx_.lock();
  bg_task_vec_.push_back(hand);
...
void SharedMemtable::SnapShot(
  ...
  TableTask task = snapshot_context;
  for (auto& iter : taskworkers_) {
    iter->addTask(task);
    iter->Notify();
  }
