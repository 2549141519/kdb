...
class BitSet {
 ...
 public:
  // 析构函数
  ~BitSet() { if(owns_data_) delete[] data; }

  constexpr BitSet() : data(nullptr), data_size(N / 8), owns_data_(true) {
    // 一个字节占 8 位
  ...
  BitSet(char* data_view, const uint32_t data_size)
      : data(data_view), data_size(data_size), owns_data_(false) {}
 ...
 private:
  ...
  bool owns_data_;  // 标志位，表示 BitSet 是否自己管理 data
};
